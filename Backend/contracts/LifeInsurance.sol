...
contract LifeInsurance is Ownable, UsingTellor {
    ...
    uint256 deadTest;  // value of more than 0 to indicate dead just for testing purpose
    ...
    function createPolicy(address _policyHolder, uint256 _coverageAmount, uint _age, bool _smoker, uint _weight) external {
        uint256 premium = calculatePremium(_coverageAmount, _age, _smoker, _weight);
        ...
        policies[_policyHolder] = Policy({
            ...
            endDate: block.timestamp + ((365 days) * 25),   // 25 yrs hardcoded for now
            isActive: true
        });
        ...
        policyholders[_policyHolder] = PolicyholderInfo({
            age: _age,
            smoker: _smoker,
            fitnessScore: _weight
        });
    }

    function calculatePremium(uint256 _covAmount, uint _age, bool _smoker, uint _weight) public pure returns (uint256) {
